...
Open Job Description is a flexible open specification started by the team behind AWS Thinkbox
Deadline for defining batch processing jobs that are portable between different scheduling system deployments.
Making jobs portable across pipelines allows technical pipeline developers to more easily and flexibly test,
adapt, and deploy the best scheduling solutions for their projects, eliminating the need for studios and developers
to recreate every render submission variant from scratch.
...
their development capacity away from improving their software.
